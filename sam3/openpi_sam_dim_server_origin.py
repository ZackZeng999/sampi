...
- Cover every required entity before considering the task visually complete.
...
class SamAgentService:
    ...
    def extract(
        ...
        *,
        ...
        try:
            ...
            # Round 2: ground functional entities from Round 1 into visible SAM prompts.
            ...
            if self._required_sources_covered(source_states) or max_rounds <= 2:
                return self._build_extract_response(
                    source_states,
                    task_decomposition=task_decomposition,
                    used_prompts=used,
            ...
            # Round 3: propose close aliases for entities that still lack a valid prompt.
            if max_rounds >= 3:
            ...
            if self._required_sources_covered(source_states) or max_rounds <= 3:
                return self._build_extract_response(
                    source_states,
                    task_decomposition=task_decomposition,
                    used_prompts=used,
            ...
            # Round 4: broaden aliases only for entities that failed every earlier round.
            if max_rounds >= 4:
                self._run_refinement_round(
                    image=image,
                    image_path=image_path,
    ...
    def _generate_source_prompts(
        ...
        *,
        ...
        user_text = (
            ...
            f"Return at most {remaining_slots} visually grounded entities in ranked order, with required entities first. "
            "Use the provided image to map each useful functional entity from task decomposition to one short visible noun "
            "phrase suitable for SAM. Preserve each entity's role, directly_contacted, state_change, inferred, parent_entity, "
            ...
            "button. Do not remove required entities, invent new functional entities, or introduce unrelated visible objects. If the exact visible form of a required entity remains uncertain, keep the functional entity itself as visual_prompt instead of omitting it. "
            "Output strict JSON only in exactly this form: "
            '{"source_prompts": [{"entity": "stove control", "source_role": "interaction_target", '
            '"visual_prompt": "stove knob", "directly_contacted": true, "state_change": "unoperated_to_operated", '
            '"inferred": true, "parent_entity": "stove", "required": true}]}'
        )
        ...
        *,
        ...
        if mode == "conservative":
            mode_text = (
                ...
                f"propose 1 to {max_candidates_per_source} close semantic or visually precise aliases. Good examples: "
                "chocolate pudding -> chocolate dessert, pudding cup, dessert; alphabet soup -> soup can, can; salad "
                "dressing -> dressing bottle, bottle."
            )
        else:
            mode_text = (
        ...
        user_text = (
            ...
            f"{mode_text} Preserve entity identity, role, parent relationship, and required status. Every candidate must "
            "remain visually and semantically tied to its entity. Do not propose unrelated visible objects. "
            "Output strict JSON only in exactly this form: "
            '{"replacements": [{"entity": "stove control", "candidates": ["control knob", "black knob"], '
    ...
    def _parse_source_prompt_candidates(self, response_text: str) -> list[dict[str, Any]]:
        ...
        for item in raw_sources:
            ...
            if not entity or not visual_prompt or entity in seen:
                continue
    ...
    def _parse_replacement_candidates(
        ...
        *,
        ...
        blob = _extract_json_blob(response_text)
        data = json.loads(blob)
        if isinstance(data, dict) and ("entity" in data or "source_prompt" in data):
        ...
        for item in raw_replacements:
            if not isinstance(item, dict):
                continue
            source = str(
                item.get("entity", item.get("source_prompt", item.get("source", item.get("original_prompt", item.get("prompt", "")))))
            ).strip().lower()
